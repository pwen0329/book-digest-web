...
vi.mock('@/lib/registration-success-email', () => ({
...
describe('/api/event/[slug]/register', () => {
  ...
  describe('Rate limiting', () => {
    it('returns 429 when rate limited', async () => {
      vi.mocked(rateLimit).mockResolvedValueOnce({ allowed: false, retryAfterMs: 60000 });
  ...
  describe('Event validation', () => {
    it('returns 404 for non-existent event', async () => {
      vi.mocked(rateLimit).mockResolvedValueOnce({ allowed: true, retryAfterMs: 0 });
