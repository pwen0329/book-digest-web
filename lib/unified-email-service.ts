...
export async function sendRegistrationSuccessEmail(input: SendRegistrationSuccessEmailInput): Promise<SendEmailResult> {
  ...
  await logEmailAudit({
    ...
    metadata: { messageId: result.messageId, provider: getEmailProviderName() },
  });
...
export async function sendTestEmail(input: SendTestEmailInput): Promise<SendEmailResult> {
  ...
  await logEmailAudit({
    ...
    subject,
    errorMessage: result.reason,
    metadata: { messageId: result.messageId, testType: input.emailType, provider: getEmailProviderName() },
  });
