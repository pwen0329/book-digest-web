...
export default async function NotebookStrip() {
  ...
  const frames = [1, 2, 3, 4, 5, 6].map((n) => `/images/notebook/notebook-0${n}.webp`);
  return (
      ...
      <div className="mx-auto max-w-6xl px-6 py-12">
        <div className="rounded-3xl border border-white/10 bg-white/5 p-4 md:p-6">
          ...
          <div className="mt-4 text-center">
            <Link href={`/${locale}/events#detox`} className={`inline-flex items-center rounded-full bg-brand-pink px-5 py-2.5 font-semibold text-white hover:brightness-110 transition-all ${locale === 'zh' ? 'tracking-widest' : ''}`}>{tModal('imIn')}</Link>
          </div>
        </div>
      </div>
    </section>
